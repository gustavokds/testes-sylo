...
app.use(express.static('./css')); //Send index.html page on GET /
...
parser.on('data', (temp) => { //Na recepção dos dados = "On data retrieving"
    //Printa o dado recebido no console
    ...
    io.sockets.emit('temp', {date: data, time: hora, temp:array[0], umid:array[1]});
});
